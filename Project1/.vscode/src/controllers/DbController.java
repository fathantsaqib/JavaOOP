...
public class DbController extends MyConfig {
    ...
    public static void getDatabase() {
        ...
        try {
            ...
            query = "SELECT NAMA, HARGA, STOCK FROM db_tabel";
    ...
    public static Produk getProdukByNama(String nama) {
        Produk produk = null;
        connection();
        query = "SELECT * FROM db_tabel WHERE NAMA=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static boolean insertData(String nama, long harga, int jumlah) {
        connection();
        query = "INSERT INTO db_tabel (NAMA, HARGA, STOCK) VALUES (?, ?, ?)";
        try {
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nama);
    ...
    public static void updateNama(int id, String nama) {
        connection();
        query = "UPDATE db_tabel SET NAMA=? WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nama);
    ...
    public static void updateHarga(int id, long harga) {
        connection();
        query = "UPDATE db_tabel SET HARGA=? WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static void updateJumlah(int id, int jumlah) {
        connection();
        query = "UPDATE db_tabel SET STOCK=? WHERE ID=?";
        try {
            preparedStatement = connection.prepareStatement(query);
    ...
    public static boolean deleteData(String nama) {
        connection();
        query = "DELETE FROM db_tabel WHERE NAMA=?";
        try {
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, nama);
